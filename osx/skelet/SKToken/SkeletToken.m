...
{
//  SkeletToken *shadowText = (__bridge SkeletToken *)context;

//  //
 // NSTextStorage *textStorage =  (SkeletTextView) shadowText.reactSuperview
...
//
  ...
  //NSLog(@"RCTMeasure not implemented %f", width);
                          ...
                          parentProperties:(NSDictionary *)parentProperties
{
  ...
  [applierBlocks addObject:^(NSDictionary<NSNumber *, RCTShadowView *> * viewRegistry) {
    SkeletText *view = (SkeletText *)viewRegistry[self.reactTag];
                                     ...
                                     useBackgroundColor:(BOOL)useBackgroundColor
{
  if (![self isTextDirty] && _cachedAttributedString) {
    NSDictionary<NSString *,id> *attr = [_cachedAttributedString
                                       fontAttributesInRange:NSMakeRange(0, _cachedAttributedString.length)];
    //NSFont *font = attr.allKeys
    if (attr && [attr.allKeys containsObject:NSFontAttributeName] && attr[NSFontAttributeName] == fontFamily) {
  ...
  for (RCTShadowView *child in [self reactSubviews]) {
    if ([child isKindOfClass:[SkeletShadowText class]]) {
      //NSLog(@"append <Token> font:%@ ", fontFamily);
      SkeletShadowText *shadowText = (SkeletShadowText *)child;
...
//      }
      ...
      RCTLogError(@"<Token> can't have any children except <Token>, <Image> or raw strings");
    }
