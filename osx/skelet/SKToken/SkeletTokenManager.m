...
@implementation SkeletShadowTextManager

RCT_EXPORT_MODULE()

- (SkeletText *)view
{
 return [SkeletText new];
}

- (SkeletShadowText *)shadowView
{
  return [SkeletShadowText new];
}
...
//                    [(RCTShadowRawText *)shadowView text]);
        ...
        if ([shadowView.viewName hasSuffix:@"TextView"]) {
          ...
          for (RCTShadowView *subview in shadowView.reactSubviews) {
            if ([subview isKindOfClass:[SkeletShadowText class]]) {
  ...
  if (textViewTagsToUpdate.count) {
    ...
    for (NSNumber *reactTag in textViewTagsToUpdate) {
      ...
      SkeletText *shadowText;
      for (SkeletText *subview in shadowTextView.reactSubviews) {
        if ([subview isKindOfClass:[SkeletShadowText class]]) {
          shadowText = subview;
          break;
        }
     ...
     // CGFloat width = shadowText.frame.size.width - (padding.left + padding.right);
      ...
      [uiBlocks addObject:^(RCTUIManager *uiManager, NSDictionary<NSNumber *, SkeletTextView *> *viewRegistry) {
        SkeletTextView *textView = viewRegistry[reactTag];
        [textView setAttributedString:shadowText.attributedString];
      }];
    }

    return ^(RCTUIManager *uiManager, NSDictionary<NSNumber *, NSView *> *viewRegistry) {
...
//- (RCTViewManagerUIBlock)uiBlockToAmendWithShadowView:(SkeletToken *)shadowView
//{
//  NSNumber *reactTag = shadowView.reactTag;
//  NSEdgeInsets padding = shadowView.paddingAsInsets;
//
//  return ^(RCTUIManager *uiManager, RCTSparseArray *viewRegistry) {
//    NSLog(@"uiBlockToAmendWithShadowView not implemented %@", reactTag);
//    SkeletTextView *text = viewRegistry[reactTag];
//    text. = padding;
//  };
//}
